...
exports.handler = async (event) => {
  ...
  try {
    ...
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        contents: [{ role: 'user', parts: [{ text: `你是熱情的旅遊AI閨蜜「小蜜」，用台灣女生口吻，加💕表情，稱「姐妹」。\n\n${message}` }] }],
    ...
    return { statusCode: 500, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }, body: JSON.stringify({ reply: '網路問題💕' }) };
  }
};
